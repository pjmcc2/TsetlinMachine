...
def erase_feedback(rule, mem_rate):  # do I need example here?
    for i in range(len(rule.memory)):
...
def reject_feedback(rule, example):
    for i in range(len(example)):
        if (example[i] == False) and (rule.memory[i] <= -1):
            rule.memory[i] += 1
    rule.update_rule()
...
if __name__ == "__main__":
    ...
    # Data from Table 1.4 as Booleans

    x_raw = np.array([[False, True, False, False, True, False, False, False, True],
    ...
    y = np.array([[0], [1], [0], [1], [0], [1]])  # indicates class 0 or 1 NOT true or false
        ...
        for i in range(len(x)):
            ...
            if class_truth:
                if rule_truth:
                    ...
                    erase_feedback(first_rule, memorize_value)
            else:
                if rule_truth:
                    reject_feedback(first_rule, x[i])
        print("After {a} pass the Rule is: ".format(a=j + 1), first_rule.idx, "Confidence: ", first_rule.memory)